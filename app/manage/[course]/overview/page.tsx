...
import { FaMailBulk, FaShare } from "react-icons/fa"
import Image from "next/image"
import Course from "@/app/courses/[course]/page"
import SmallCoursePreview from "@/components/CoursePreview"
...
export default async function Page({
    ...
    return <div className="bg-zinc-100 min-h-screen">
        <ScrollAbleActions/>
        <CourseDetails parameters={params}/>
    </div>
  }

export const CourseDetails = ({parameters}:any) => {
    return (
      ...
      <Card className=" rounded-[2px] w-full  bg-primary shadow-sm border-none  text-white">
        <CardContent className="p-3 md:flex gap-4">
          ...
          <div>
            <CardTitle className="text-offblack my-4">When and where</CardTitle>
            
            {/* Info Div */}
            <div className="flex gap-3 mb-4 items-start">
              <div className="bg-zinc-800 rounded-lg p-2 w-8 h-8 flex flex-col items-center justify-center">
              <span className="text-[0.4rem] text-primary text-center bg-opacity-50 bg-zinc-400 w-8 rounded-t-lg  uppercase">DES</span>

                <span className="text-md">26</span>
              </div>
              <div className="space-y-1 text-offblack flex flex-col">
                <span>torsdag 26. des.</span>
                <span className="text-zinc-400 text-sm">18:00 - 29. des., 19:00 GMT</span>
              </div>
            </div>
      ...
      <Card className="bg-primary text-offblack border-none">
        <CardContent>
          <CardHeader>
            ...
            <CardDescription>These are the current invites that has been sent!</CardDescription>
  ...
  return (
    <Card className="my-4 shadow-md rounded-[2px]">
      <CardContent className="flex items-center gap-4 text-muted-foreground">
        ...
        <div>
          <CardTitle  className="text-xl">You haven't sent any invites</CardTitle>
          <CardDescription>You can send invites to friends, family and others you may think are interested in the course!</CardDescription>
        </div>
      </CardContent>
    </Card>
  )
...
export const ScrollAbleActions = () =>{
    return(
        <div className="overflow-scroll flex justify-evenly gap-2 p-2 w-full">
...
export const ActionComponent = ({type}:{type: ActionVariants}) =>{
   if(type == "invite"){
    return(
